...
O fluxo de contribuições acima se materializa no **BRAIN PRO**, um caderno de
aprendizagem executável sobre cérebro, modelagem matemática e sistemas. A
aplicação combina um motor determinístico em Rust/Wasm com uma leitura espacial
...
[navegar pela documentação](docs/README.md)
...
| experiência interativa | R10-F funcional incorporada: modos, paleta, foco/contexto, proveniência, câmera reversível, vistas salvas e trilha de escalas |
...
| último corte de interação | R10-F implementa UI-031..037 e UX-003; cobertura integral de toque/leitor de tela e promoção agregada continuam pendentes |
...
- **(R10-F, funcional)**: modos `guided`, `explorer` e `laboratory`, paleta modal
  ...
  escada de escala pulável, todos sem alterar Worker, ABI, snapshot ou solver;
...
| navegação R10-F → ciência | `src/presentation-navigation.ts` schema 1 guarda somente poses/trilha; câmera, escala e foco não entram em preset, ABI ou Worker | `npm run verify:presentation-navigation` prova `Escape`, cubo SVG/DOM, pulo, movimento reduzido e invariância dos cinco hashes com motor congelado |
...
a vascularização e a anatomia são fenomenológicos ou ilustrativos; não formam
um atlas calibrado, uma simulação clínica ou uma reprodução digital do Kandel.
Realismo visual não é tratado como evidência biológica.
...
| [Auditoria R10-F](docs/audits/0.10/AUDIT_0.10_R10_F.md) | implementação funcional de UI/interação, cobertura restante e promoção ainda pendente |
...
| [Auditorias](docs/audits/) | evidências organizadas por versão |

O histórico Git verificável começa em **2026-07-20**. Decisões encerradas ficam
nas auditorias e documentos legados, sem transformar o README em changelog.
...
- [Referências científicas do projeto](docs/references/REFERENCES.md)
