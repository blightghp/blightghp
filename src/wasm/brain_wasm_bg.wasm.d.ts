/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_wasmlaminarengine_free: (a: number, b: number) => void;
export const __wbg_wasmneuralengine_free: (a: number, b: number) => void;
export const wasmlaminarengine_excitatory: (a: number) => [number, number];
export const wasmlaminarengine_inhibitory: (a: number) => [number, number];
export const wasmlaminarengine_new: (a: number) => [number, number, number];
export const wasmlaminarengine_reset: (a: number) => void;
export const wasmlaminarengine_schema_version: () => number;
export const wasmlaminarengine_step_with_layer_four_drive: (a: number, b: number) => [number, number];
export const wasmlaminarengine_tick: (a: number) => bigint;
export const wasmneuralengine_activations: (a: number) => [number, number];
export const wasmneuralengine_advance_scheduled_to: (a: number, b: number) => [number, number];
export const wasmneuralengine_advance_to: (a: number, b: number, c: number, d: number, e: number) => [number, number];
export const wasmneuralengine_cell_adaptation_amperes: (a: number) => [number, number];
export const wasmneuralengine_cell_ampa_amperes: (a: number) => [number, number];
export const wasmneuralengine_cell_blend: (a: number) => number;
export const wasmneuralengine_cell_dendrite_volts: (a: number) => [number, number];
export const wasmneuralengine_cell_excitatory_inhibitory_ratio: (a: number) => number;
export const wasmneuralengine_cell_field_vertex: (a: number) => number;
export const wasmneuralengine_cell_firing_rate_hz: (a: number) => number;
export const wasmneuralengine_cell_first_spike_seconds: (a: number) => number;
export const wasmneuralengine_cell_gabaa_amperes: (a: number) => [number, number];
export const wasmneuralengine_cell_gabab_amperes: (a: number) => [number, number];
export const wasmneuralengine_cell_kinds: (a: number) => [number, number];
export const wasmneuralengine_cell_membrane_volts: (a: number) => [number, number];
export const wasmneuralengine_cell_nmda_amperes: (a: number) => [number, number];
export const wasmneuralengine_cell_spike_event_cell_ids: (a: number) => [number, number];
export const wasmneuralengine_cell_spike_event_end_tick: (a: number) => number;
export const wasmneuralengine_cell_spike_event_hash: (a: number) => [number, number];
export const wasmneuralengine_cell_spike_event_schema_version: (a: number) => number;
export const wasmneuralengine_cell_spike_event_start_tick: (a: number) => number;
export const wasmneuralengine_cell_spike_event_time_offsets_seconds: (a: number) => [number, number];
export const wasmneuralengine_cell_spiked: (a: number) => [number, number];
export const wasmneuralengine_cell_state_hash: (a: number) => [number, number];
export const wasmneuralengine_chemical_cleared_moles: (a: number) => [number, number];
export const wasmneuralengine_chemical_cleft_concentration: (a: number) => [number, number];
export const wasmneuralengine_chemical_cleft_moles: (a: number) => [number, number];
export const wasmneuralengine_chemical_latest_release_moles: (a: number) => [number, number];
export const wasmneuralengine_chemical_latest_release_time_seconds: (a: number) => [number, number];
export const wasmneuralengine_chemical_presynaptic_spike_counts: (a: number) => [number, number];
export const wasmneuralengine_chemical_receptor_bound_moles: (a: number) => [number, number];
export const wasmneuralengine_chemical_receptor_occupancy_fraction: (a: number) => [number, number];
export const wasmneuralengine_chemical_release_event_indices: (a: number) => [number, number];
export const wasmneuralengine_chemical_solver_step_index: (a: number) => bigint;
export const wasmneuralengine_chemical_state_hash: (a: number) => [number, number];
export const wasmneuralengine_chemical_time_seconds: (a: number) => number;
export const wasmneuralengine_chemical_total_released_moles: (a: number) => [number, number];
export const wasmneuralengine_chemical_vesicle_available_fraction: (a: number) => [number, number];
export const wasmneuralengine_chemical_vesicle_utilization_fraction: (a: number) => [number, number];
export const wasmneuralengine_corticothalamic_state_hash: (a: number) => [number, number];
export const wasmneuralengine_field_excitatory: (a: number) => [number, number];
export const wasmneuralengine_field_inhibitory: (a: number) => [number, number];
export const wasmneuralengine_field_node_indices: (a: number) => [number, number];
export const wasmneuralengine_field_wave_activity: (a: number) => [number, number];
export const wasmneuralengine_firing_rate: (a: number) => number;
export const wasmneuralengine_laminar_excitatory: (a: number) => [number, number];
export const wasmneuralengine_laminar_inhibitory: (a: number) => [number, number];
export const wasmneuralengine_layer6_feedback: (a: number) => number;
export const wasmneuralengine_mean_weight: (a: number) => number;
export const wasmneuralengine_new: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number, k: number, l: number, m: number, n: number, o: number, p: number, q: number, r: number, s: number, t: number, u: number, v: number, w: number, x: number, y: number, z: number, a1: number, b1: number) => [number, number, number];
export const wasmneuralengine_potentials: (a: number) => [number, number];
export const wasmneuralengine_relay_drive_to_l4: (a: number) => number;
export const wasmneuralengine_reset: (a: number, b: number) => void;
export const wasmneuralengine_schedule_plasticity: (a: number, b: number, c: number, d: number) => [number, number];
export const wasmneuralengine_schedule_stimulus: (a: number, b: number, c: number, d: number, e: number) => [number, number];
export const wasmneuralengine_schema_version: () => number;
export const wasmneuralengine_signal_inhibitory: (a: number) => [number, number];
export const wasmneuralengine_signal_progress: (a: number) => [number, number];
export const wasmneuralengine_signal_strength: (a: number) => [number, number];
export const wasmneuralengine_signal_synapse_ids: (a: number) => [number, number];
export const wasmneuralengine_spikes: (a: number) => number;
export const wasmneuralengine_state_hash: (a: number) => [number, number];
export const wasmneuralengine_thalamic_rebound: (a: number) => number;
export const wasmneuralengine_thalamic_relay: (a: number) => number;
export const wasmneuralengine_thalamic_trn: (a: number) => number;
export const wasmneuralengine_tick: (a: number) => number;
export const wasmneuralengine_time_seconds: (a: number) => number;
export const wasmneuralengine_weights: (a: number) => [number, number];
export const __wbindgen_externrefs: WebAssembly.Table;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __externref_table_dealloc: (a: number) => void;
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __wbindgen_start: () => void;
