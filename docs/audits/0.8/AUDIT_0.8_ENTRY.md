...
> [ARCHITECTURE.md, “Achados da auditoria 0.8 reavaliados”](../../specifications/ARCHITECTURE.md#achados-da-auditoria-08-reavaliados).
...
1. o roadmap exclui explicitamente atlas anatômico, morfologia
   multicompartimental e tractografia do caminho até a 1.0 (`../../planning/ROADMAP.md:188`);
...
| coerência de versão | `../../../package.json`, `index.html`, `../../planning/ROADMAP.md`, `PLAN_0.7.md` e `../0.7/AUDIT_0.7.md` concordam em 0.7.0 |
...
[AUDIT_0.7.md](../0.7/AUDIT_0.7.md). As afirmações abaixo sobre o motor vêm de leitura
...
| # | Achado | Severidade |
| :-- | :-- | :-- |
| P1 | A 0.8 não tem plano de cortes equivalente a [PLAN_0.6](../../legacy/plans/PLAN-0.6.md) e [PLAN_0.7](../../legacy/plans/PLAN-0.7.md). A fase existe como uma linha de tabela (`../../planning/ROADMAP.md:37`) e três itens abertos de preparação (`../../planning/ROADMAP.md:153-156`). As duas fases anteriores só começaram com plano escrito antes do código. | alto |
| P2 | O programa visual pedido não aparece em nenhum documento. Plano de corte, película transparente, ampliação de neurônio, espraiamento espacial de neurotransmissor e sistema de cor por substância não têm uma linha sequer; morfologia aparece apenas como negação, nas ressalvas do que o modelo não faz. E não é omissão de redação — `../../planning/ROADMAP.md:182-188` declara o contrário, mantendo regiões procedurais, casca convexa e ausência de atlas e morfologia como limites assumidos até a 1.0. | bloqueio |
| P3 | `../../specifications/ARCHITECTURE.md:49` ainda intitula a seção principal como arquitetura 0.6, e `../../specifications/ARCHITECTURE.md:15` descreve `main.ts` no estado 0.4 — "ainda concentra relógio, cena, renderização, HUD, captura e controles" —, o que deixou de ser verdade quando as camadas de render foram extraídas. | médio |
| P4 | A própria arquitetura define o gatilho para criar `src/render/`: três camadas extraídas de `main.ts` (`../../specifications/ARCHITECTURE.md:462`). O gatilho disparou — existem `render-layers.ts`, `laminar-layer.ts` e `cell-layer.ts` — e o diretório não foi criado. As três camadas convivem soltas em `src/`, sem tipo comum e sem a interface `RenderLayer` que o documento especifica em `../../specifications/ARCHITECTURE.md:404`. | médio |
| P5 | `inference.ts` e o HUD bayesiano continuam ocupando o painel inferior inteiro da Visão Geral. A posterior é calculada em TypeScript e entra no motor como `confidence` do drive (`main.ts:218`), fora do contrato `ExperimentEncoder`/`ExperimentDecoder` previsto em `../../specifications/ARCHITECTURE.md:357`. Nenhum teste cobre seu efeito sobre a dinâmica. | médio |

### Evidência e gates

| # | Achado | Severidade |
| :-- | :-- | :-- |
| E1 | `runtime-audit.json` registra cadência, contraste e contadores, mas **não** registra hardware, navegador, preset, número de nós, sinapses nem vértices — campos que `../../quality/VALIDATION.md:96` exige de todo relatório de desempenho. O artefato não satisfaz o critério que o documento define. | alto |
| E2 | O mesmo artefato publica `frameCpuMs.p95 = 81,5 ms` e média de 37,9 ms, e a captura de Visão Geral exibe 10 FPS. É ambiente headless com rasterização por software, e [AUDIT_0.7.md](../0.7/AUDIT_0.7.md) diz isso. O problema é outro: **não existe nenhuma medição em hardware real**. O alvo de 60 Hz de `../../quality/VALIDATION.md:108` não tem evidência que o sustente nem que o refute, e o programa visual vai multiplicar o custo gráfico. | alto |
| E3 | O gate visual mede contraste de texto contra o painel. Ele não verifica o critério vizinho, declarado em `../../quality/VALIDATION.md:115`: legibilidade sem depender apenas de cor. Hoje excitatório e inibitório têm geometria idêntica e só diferem em matiz, nas três vistas. | médio |
| E4 | `../../quality/VALIDATION.md:117` exige confirmar que cada pulso visível aponta para um evento publicado. Isso é verdadeiro por leitura do código, mas nenhum teste amarra cor ou posição renderizada de volta ao estado do snapshot. É a única linha da validação gráfica sem execução. | médio |
...
| :-- | :-- | :-- | :-- |
| Camadas progressivas com anatomia realista | envoltório convexo por região, nuvem de pontos procedural | não; explicitamente excluído em `../../planning/ROADMAP.md:184,188` | superfície dobrada com proveniência declarada; separação entre matéria e emissão no render |
...
| Correntes sinápticas e trocas neuroquímicas no neurônio | quatro medidores em painel, sem sinal e sem lugar | parcialmente: abas Sinapse e Bioquímica em `../../planning/ROADMAP.md:37` | corrente com sinal e direção; estado de fenda, vesícula e ocupação |
...
1. Criar `src/render/` com a interface `RenderLayer` já especificada em
   `../../specifications/ARCHITECTURE.md:404`, e mover as três camadas para dentro. Fecha P4.
...
8. Registrar um baseline de desempenho em hardware real, com o formato de
   `../../quality/VALIDATION.md:96`, antes de aumentar o custo gráfico. Fecha E2.
...
10. Atualizar o cabeçalho e a descrição de `main.ts` em `../../specifications/ARCHITECTURE.md`.
    Fecha P3.
...
12. Escrever o contrato de apresentação — o que este relatório entrega em
    [proposta visual arquivada](../../legacy/specs/VISUAL-SPEC-v0.8-proposal.md) — e substituir os limites assumidos do
...
[proposta de roadmap arquivada](../../legacy/roadmaps/ROADMAP-NEXT-v0.8-proposal.md) e o desenho da leitura visual na
[proposta visual arquivada](../../legacy/specs/VISUAL-SPEC-v0.8-proposal.md).
